...
void serMain(byte command, byte* input_serial_buffer) {
  ...
  case 2:
  {
	...
	case 23:
	{
			   ...
			   joystickSet(mode);
			   debug.ser("incoming request for joystick mode: ");
			   debug.serln(mode);
			   response(true);
			   break;
	}
	...
	case 25:
		debug.serln("Sending motors to start positions");
		sendAllToStart();
		response(true);
		break;
...
void serMotor(byte subaddr, byte command, byte* input_serial_buffer) {
	...
	case 19:
		...
		debug.ser("Lead-in:");
		debug.serln(motor[subaddr - 1].planLeadIn());
		if (!graffik_mode)
...
void serKeyFrame(byte command, byte* input_serial_buffer){
	...
	case 11:
	{
		...
		if (in_val >= 0){				   		
			...
			debug.ser("Axis ");
			debug.ser(axis);
			debug.ser(" -- Setting key frame count: ");
			debug.serln(kf[axis].getKFCount());
		}		   			  
	...
	{		
		...
		debug.ser("Axis ");
		debug.ser(axis);
		debug.ser(" -- Setting absicssa ");
		debug.ser(frame);
		debug.ser(": ");
		debug.serln(kf[axis].getXN(frame));
	...
	case 13:
	{
		...
		debug.ser("Axis ");
		debug.ser(axis);
		debug.ser(" -- Setting position ");
		debug.ser(frame);
		debug.ser(": ");
		debug.serln(kf[axis].getFN(frame));
	...
	case 14:
	{
		...
		debug.ser("Axis ");
		debug.ser(axis);
		debug.ser(" -- Setting velocity ");
		debug.ser(frame);
		debug.ser(": ");
		debug.serln(kf[axis].getDN(frame));
	...
	case 16:
	{
		...
		debug.ser(" Stop: ");
		debug.serln(motor[axis].stopPos());
	...
	case 102:
	{
		...
		debug.ser("The position at time ");
		debug.ser(in_val);
		debug.ser(": ");
		debug.serln(ret);
